...
echo.
echo ERROR: No javaw.exe found at: %JAVA_EXE%
goto common_error
...
set JAVA_OPTS=
if x == x%JAVA_OPTS% set JAVA_OPTS=-Xmx512m
set JAVA_OPTS=%JAVA_OPTS% -Dprogram.name=%GRIFFON_APP_NAME% -Dgriffon.start.dir=%DIRNAME%..\ -Djava.library.path=%NATIVE_LIBRARY_PATH%

@rem Execute App
start /b %JAVA_EXE% %JAVA_OPTS% -splash:splash.png -classpath "%CP%" griffon.swing.SwingApplication %CMD_LINE_ARGS%
